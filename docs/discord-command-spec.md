...
7. Bot이 모든 asset을 검증하고 local storage 또는 storage provider에 저장한 뒤 target별 진행 상태 메시지를 표시한다.
...
Instagram/Facebook은 Meta App Review/Advanced Access와 publishing permission이 준비된 target만 자동 업로드한다. 권한이 없는 target은 manual upload packet 없이 업로드하지 않고 `skipped` 상태와 API access 미준비 사유를 반환한다.
...
다음 조치: API access와 publishing permission을 준비한 뒤 다시 시도해주세요.
```
