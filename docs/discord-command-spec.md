...
3. Bot이 ephemeral 상태 메시지를 만들고 `이미지/영상 파일을 이 메시지에 reply로 업로드해주세요.`라고 안내한다.
4. 사용자는 안내 메시지에 reply하거나 bot이 만든 thread에 attachment를 업로드한다.
5. Bot은 같은 user, 같은 channel/thread, 같은 pending post id에 연결된 message attachment만 수집한다.
6. Bot이 attachment 수집 timeout 안에 받은 파일을 검증하고 업로드 flow를 계속 진행한다.
...
- 사용자가 잘못된 channel에 올리거나 reply/thread 연결이 없으면 파일을 무시하고 pending 안내 메시지만 갱신한다.
...
- 허용 파일은 `image/*` 또는 `video/mp4`다. 구현은 filename만 믿지 말고 Discord attachment `content_type`, CDN response `Content-Type`, 가능한 경우 file signature를 함께 확인한다.
...
- 동일한 pending post에 modal submit 또는 attachment reply가 중복 도착하면 첫 유효 submit만 처리하고 나머지는 "이미 처리 중"으로 응답한다.

Validation 실패 응답 예시:

```text
게시물 파일을 확인해주세요.
- logo.svg: image/* 또는 mp4만 업로드할 수 있습니다.
- clip.mov: mp4만 지원합니다.
