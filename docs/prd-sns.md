...
8. Bot이 입력을 검증한다.
9. Bot이 채널별 업로드를 실행한다. Homepage는 Creatorlink admin browser automation으로 notice/gallery 포스팅을 완료하고, 권한이 준비되지 않은 Instagram/Facebook target은 업로드 없이 건너뛴다.
...
3. 승인 전에는 Instagram/Facebook target을 업로드하지 않고 `skipped`로 기록한다. 수동 업로드 패킷도 생성하지 않는다.
4. App Review 제출에는 실제 `/post` flow에서 Meta Login, Page/Instagram 선택 또는 확인, 게시 실행, 결과 URL 반환까지 보여주는 테스트 path와 screencast가 필요하다.

권한 미준비 target 처리:

1. Meta API access, access token, App Review/Advanced Access, publishing permission이 준비되지 않은 Instagram/Facebook target은 `skipped`로 기록한다.
2. `skipped` target은 자동 업로드를 시도하지 않고, title/content/assets 기반 manual upload packet도 생성하지 않는다.
3. 사용자-facing 결과에는 해당 target이 권한 미준비로 건너뛰어졌고, App Review/permission 준비 후 다시 실행해야 한다고 표시한다.
4. Homepage가 함께 선택되어 성공하면 parent는 `partial_success`다. Instagram/Facebook만 선택했고 모두 `skipped`이면 parent는 `failed`가 아니라 `draft`로 유지해 업로드가 수행되지 않았음을 나타낸다.
...
| SNS-FE-005 | Instagram upload adapter를 제공한다. App Review/Advanced Access 승인 전에는 업로드하지 않고 `skipped`를 반환한다. | P1 |
| SNS-FE-006 | Facebook upload adapter를 제공한다. App Review/Advanced Access 승인 전에는 업로드하지 않고 `skipped`를 반환한다. | P1 |
...
3. 선택된 모든 target이 `success`이면 parent는 `success`다.
4. 모든 target 처리가 끝났고 하나 이상이 `success`이며 나머지가 `failed` 또는 `skipped`이면 parent는 `partial_success`다.
5. 선택 target이 모두 `skipped`이면 parent는 `draft`로 유지한다. 이 상태는 업로드 작업이 수행되지 않았고 권한 준비 후 다시 실행해야 함을 의미한다.
6. 모든 active target이 `failed`이거나 제출 전 validation failure로 target work가 시작되지 못하면 parent는 `failed`다.
...
Discord attachment CDN URL은 일정 시간 뒤 접근이 만료될 수 있으므로 provider upload는 `source_attachment_url`을 장기 다운로드 링크로 사용하지 않는다. Bot은 modal submit 또는 attachment 수집 직후 파일을 local storage 또는 향후 durable storage adapter에 복사하고, provider upload에는 `storage_url`을 사용한다. `storage_url`이 없고 source URL도 만료된 경우 해당 target은 재업로드가 필요한 실패로 표시한다.
...
7. Instagram/Facebook platform upload가 설정되지 않았으면 조용히 실패하지 말고 권한 미준비로 `skipped` 처리한다. 수동 업로드 패킷은 생성하지 않는다.
...
12. Provider upload는 만료 가능한 Discord `source_attachment_url`을 장기 다운로드 링크로 쓰지 않는다. 저장된 `storage_url`/local path를 사용한다.
