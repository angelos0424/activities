...
4. 모든 target 처리가 끝났고 하나 이상이 `success`이고 하나 이상이 `failed` 또는 `skipped`이면 parent는 `partial_success`다.
5. 모든 선택 target이 `failed` 또는 `skipped`이거나 제출 전 validation failure로 target work가 시작되지 못하면 parent는 `failed`다.
