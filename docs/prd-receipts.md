...
영수증 서비스는 로컬에 설치한 Discord bot을 `#receipt` 채널에서 사용한다. 사용자가 지출자 이름을 입력하면 bot이 Google Sheets의 사람/단체 정보를 찾아 송금 이력 row를 만들고, 영수증 이미지를 로컬 파일시스템에 저장해 정산 대기 상태를 관리한다.
...
| account | no | copied from people sheet |
| amount | no | optional until OCR/manual entry is defined |
...
| created_at | yes | timestamp |
| completed_at | no | filled manually or by sync later |

### Local Bot Config

| Field | Required | Notes |
| --- | --- | --- |
| people_sheet_url | yes | configurable by slash command |
| transfers_sheet_url | yes | configurable by slash command |
| receipt_storage_dir | yes | defaults to `data/receipts` |
| updated_by | yes | Discord admin user id |
| updated_at | yes | timestamp |

### Local File Access Model

영수증 이미지는 `data/receipts/{yyyy}/{MM}/{generated_filename}`에 저장한다.
...
8. 저장 경로는 `data/receipts/{yyyy}/{MM}/{generated_filename}` 형식을 사용한다.
